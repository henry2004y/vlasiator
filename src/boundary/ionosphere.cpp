...
/*!\file ionosphere.cpp
 * \brief Implementation of the class BoundaryCondition::Ionosphere to handle cells classified as
 * boundarytype::IONOSPHERE.
 */
...
void Ionosphere::addParameters() {
   ...
   Readparameters::add("ionosphere.geometry",
   ...
   for (uint i = 0; i < getObjectWrapper().particleSpecies.size(); i++) {
      ...
      Readparameters::add(pop + "_ionosphere.VY0",
                          "Bulk velocity of ionospheric distribution function in X direction (m/s)", 0.0);
      Readparameters::add(pop + "_ionosphere.VZ0",
                          "Bulk velocity of ionospheric distribution function in X direction (m/s)", 0.0);
      Readparameters::add(pop + "_ionosphere.fluffiness",
   ...
   if (Parameters::xcells_ini == 1) {
      if (Parameters::ycells_ini == 1) {
         if (Parameters::zcells_ini == 1) {
            ...
            std::cerr
                << __FILE__ << ":" << __LINE__ << ":"
                << "What do you expect to do with a single-cell simulation of ionosphere boundary type? Stop kidding."
                << std::endl;
            abort();
            // end of X,Y,Z
         } else {
...
void Ionosphere::fieldSolverBoundaryCondMagneticFieldProjection(
   ...
   if ((technicalGrid.get(i, j, k)->boundaryLayer == 2) ||
       ((technicalGrid.get(i, j, k)->boundaryLayer == 1) &&
        ((technicalGrid.get(i, j, k)->SOLVE & compute::BX) != compute::BX))) {
      bGrid.get(i, j, k)->at(fsgrids::bfield::PERBX) = BdotN * normalDirection[0];
   }
   if ((technicalGrid.get(i, j, k)->boundaryLayer == 2) ||
       ((technicalGrid.get(i, j, k)->boundaryLayer == 1) &&
        ((technicalGrid.get(i, j, k)->SOLVE & compute::BY) != compute::BY))) {
      bGrid.get(i, j, k)->at(fsgrids::bfield::PERBY) = BdotN * normalDirection[1];
   }
   if ((technicalGrid.get(i, j, k)->boundaryLayer == 2) ||
       ((technicalGrid.get(i, j, k)->boundaryLayer == 1) &&
        ((technicalGrid.get(i, j, k)->SOLVE & compute::BZ) != compute::BZ))) {
